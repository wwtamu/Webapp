...
import com.project.SpringContext;
...
public class ControlledVocabulary extends BaseEntity {
    ...
    public Set<Object> getDictionary() {
        ...
        if(!isEntityProperty()) {
            ...
            try {                
                EntityService entityService = SpringContext.bean(EntityService.class);
                
                entityService.getControlledVocabulary(entityName, new HashMap<String, Map<String, String>>(), name).parallelStream().forEach(property -> {
                    values.add(property);
                });                
            }
